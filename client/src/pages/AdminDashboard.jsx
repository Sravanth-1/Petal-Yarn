...
const AdminDashboard = () => {
    ...
    const handleCreateEvent = async (e) => {
        ...
        try {
            ...
            alert(error.response?.data?.message || 'Something went wrong while creating this little creation');
        }
    };

    const handleDeleteEvent = async (id) => {
        if (window.confirm('Are you sure you want to remove this crochet creation?')) {
            try {
                await api.delete(`/events/${id}`);
                fetchData();
            } catch (error) {
                alert('Something went wrong while removing the creation');
            }
        }
    };
    ...
    const handleConfirmBooking = async (id, paymentStatus) => {
        try {
            ...
            alert(error.response?.data?.message || 'Something went wrong while confirming this order');
        }
    };

    const handleCancelBooking = async (id) => {
        if (window.confirm('Cancel this customer\'s order request?')) {
            try {
                await api.delete(`/bookings/${id}`);
                fetchData();
            } catch (error) {
                alert(error.response?.data?.message || 'Something went wrong while cancelling this order');
            }
        }
    };

    if (loading) return <div className="text-center py-20 text-xl font-semibold">Preparing your cozy little shop...</div>;
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="bg-black text-white rounded-2xl p-6 sm:p-8 mb-8 shadow-lg flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-extrabold mb-2">Cozy Shop Dashboard ♡</h1>
                    <p className="text-gray-300">Manage your handmade creations and customer orders.</p>
                </div>
                ...
                >
                    {showEventForm ? 'Cancel Creation' : '+ Add New Crochet ♡'}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-sm font-bold uppercase tracking-wider mb-1">Sweet Earnings</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-sm font-bold uppercase tracking-wider mb-1">Happy Customers</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-sm font-bold uppercase tracking-wider mb-1">Waiting Orders</p>
            ...
            {showEventForm && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 animation-slideDown">
                    <h2 className="text-2xl font-bold mb-6 text-gray-800">Add a New Handmade Creation ♡</h2>
                    ...
                    <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <input
                            required
                            type="text"
                            placeholder="Category (e.g., Bags, Plushies)"
                            className="border px-4 py-3 rounded-lg focus:ring-2 focus:ring-gray-700 outline-none transition"
                        ...
                        <div className="md:col-span-2">
                            <input
                                type="text"
                                placeholder="Photo URL (Provide any direct link to an image)"
                        ...
                        <textarea
                            required
                            placeholder="Tell us about this adorable handmade creation..."
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <ul className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
                            {events.length === 0 ? (
                                <li className="p-6 text-gray-500 text-center">No handmade creations yet. ♡</li>
                            ) : (
                                events.map(event => (
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                                                ...
                                                <span className="flex items-center gap-1 font-medium">
                                                    ...
                                                    {event.availableSeats}/{event.totalSeats} pieces
                                                </span>
                                            </div>
                                        </div>
                ...
                <div className="flex flex-col">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <ul className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
                            {bookings.length === 0 ? (
                                <li className="p-6 text-gray-500 text-center">No orders yet. ♡</li>
                            ) : (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-3">
                                            <h4 className="font-bold text-gray-900 text-lg leading-tight">
                                                {booking.eventId?.title || 'Removed Creation'}
                                            </h4>

                                            <div className="flex flex-col gap-1 items-end shrink-0 ml-4">
                                                <span className={`px-2 py-1 text-[10px] font-black rounded uppercase tracking-wider ${booking.status === 'confirmed' ? 'bg-green-100 text-green-700' : booking.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                    {booking.status}
                                                </span>
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100 text-sm">
                                            <p className="text-gray-700 flex items-center gap-2 mb-1">
                                                <span className="font-bold w-16 text-gray-500 uppercase text-xs">Customer:</span>
                                            ...
                                            <p className="text-gray-700 flex items-center gap-2 mb-1">
                                                <span className="font-bold w-16 text-gray-500 uppercase text-xs">Ordered:</span>
                                            ...
                                            {booking.eventId && (
                                                <p className="text-gray-700 flex items-center gap-2 mt-2 pt-2 border-t border-gray-200">
                                                    <span className="font-bold w-16 text-gray-500 uppercase text-xs">Pieces:</span>
                                                    <span className={`font-bold ${booking.eventId.availableSeats > 0 ? 'text-green-600' : 'text-red-500'}`}>
                                                        {booking.eventId.availableSeats}
                                                    </span> remaining of {booking.eventId.totalSeats}
                                                </p>
                                            )}
                                        </div>
